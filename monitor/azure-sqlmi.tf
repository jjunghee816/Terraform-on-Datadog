resource "datadog_monitor" "azure-sqlmi-status" {
  ...
  type                = "metric alert"
  query               = "max(last_15m):avg:azure.sql_managedinstances.status{*} by {name,type} < 1"
  notify_no_data      = true
  no_data_timeframe   = 0
  evaluation_delay    = 0
...
resource "datadog_monitor" "azure-sqlmi-cpu" {
  ...
  type                = "metric alert"
  query               = "avg(last_5m):avg:azure.sql_managedinstances.avg_cpu_percent {*} by {name} >= 80"
  notify_no_data      = true
...
resource "datadog_monitor" "azure-sqlmi-storage" {
  ...
  type                = "metric alert"
  query               = "max(last_5m):avg:azure.sql_managedinstances.storage_space_used_mb{*} by {name,type} / avg:azure.sql_managedinstances.reserved_storage_mb{*} by {name,type} * 100 >= 60"
  notify_no_data      = true
  no_data_timeframe   = 10
  evaluation_delay    = 0
  require_full_window = false
