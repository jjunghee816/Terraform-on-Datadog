resource "datadog_dashboard" "reportboard_vm_azuresql" {
  ...
  widget {
    note_definition {
      content          = "# Monthly Resource Usage"
  ...
  widget {
    query_table_definition {
      request {
        q                 = "avg:azure.sql_servers_databases.cpu_percent{*} by {name}"
        aggregator        = "avg"
        alias             = "AVG CPU(%)"
        order             = "desc"
        limit             = "100"
      ...
      request {
        q                 = "max:azure.sql_servers_databases.cpu_percent{*} by {name}"
        aggregator        = "max"
        alias             = "MAX CPU(%)"
        order             = "desc"
        limit             = "100"
      ...
      request {
        q                 = "avg:azure.sql_servers_databases.sql_server_process_memory_percent{*} by {name}"
        aggregator        = "avg"
        alias             = "AVG MEM(%)"
        order             = "desc"
        limit             = "100"
      ...
      request {
        q                 = "max:azure.sql_servers_databases.sql_server_process_memory_percent{*} by {name}"
        aggregator        = "max"
        alias             = "MAX MEM(%)"
        order             = "desc"
        limit             = "100"
